...
//	var BANN_REMAIN_SEC = 3000;
	...
	var $tooltip   = $('#ubotArea .bg_tooltip'); // 툴팁(말풍선)
...
//		console.info("watchScore : ", watchScore);
		...
		//  배너 세팅
		$ubotArea.attr('class', 'ubot_area '+bannerClass);
	...
	_ubot.openBanner = function() {
		...
		case 0:case 1:case 4:case 5:case 8:case 9:case 12:case 13:case 16:case 17:case 20:case 21:
			bannerClass = 'onCard'; bannerNum = 2;
			break;
...
//			alert('[DEV] 상담사 호출 배너 : ' + openCs);
	...
	_ubot.closeChat = function(){
		_ubot.chatOpened = false;
		$('.ubot_cont').slideUp(300, function() {
			$('.ubot_cont').find('iframe').remove();
			
			if( _ubot.chatbotQuery != defaultChatbotQuery ) {
				_ubot.chatbotQuery = defaultChatbotQuery;
			}
	...
	function init() {
		...
		jumpSet();// 아이콘 콩콩
		_ubot.runJump = setInterval(jumpSet, 10000);// 아이콘 콩콩 10초 간격 인터벌
	...
	function bindEvent() {
		...
		$(window).resize(function() {
			Ubot.ubotContHeight = $(window).height() - 206;
//			if(chatBotUse === 'Y') {
				$('.ubot_cont iframe').height(Ubot.ubotContHeight);
//			}
		});
	}; // bindEvent
	
	function loadUbotIframe() {
		var chatUrl = chatbotDefaultUrl + _ubot.chatbotQuery.toString();
		$('.ubot_cont iframe').remove();
		$('.ubot_cont').append('<iframe src="'+chatUrl+'" width="770px" height="'+ubotContHeight+'" frameborder="0"></iframe>');
	}
